...
import pytest
import numpy as np
from app.retriever.fusion import (
    ScoreNormalizer, 
    HybridFusion, 
    MMRDiversifier,
    HybridRetriever
...
class TestHybridFusion:
    ...
    def test_fusion_alpha_extremes(self):
        ...
        results_semantic = fusion_semantic.fuse_results(semantic_results, keyword_results)
...
class TestMMRDiversifier:
    ...
    def test_mmr_basic(self):
        ...
        for chunk_id in embeddings:
            embeddings[chunk_id] = embeddings[chunk_id] / np.linalg.norm(embeddings[chunk_id])
        
        query_embedding = np.array([1.0, 0.0, 0.0], dtype=np.float32)
        
        # Request 3 results
        mmr_results = mmr.diversify(ranked_results, embeddings, query_embedding, max_results=3)
    ...
    def test_mmr_more_results_than_candidates(self):
        ...
        mmr_results = mmr.diversify(ranked_results, embeddings, query_embedding, max_results=5)
    ...
    def test_mmr_zero_max_results(self):
        ...
        results = mmr.diversify(ranked_results, embeddings, query_embedding, max_results=0)
        assert results == []
