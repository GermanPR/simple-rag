...
class ScoreNormalizer:
    ...
    def normalize_score_dict(score_dict: Dict[int, float]) -> Dict[int, float]:
        ...
        return dict(zip(chunk_ids, normalized_scores))
...
class HybridRetriever:
    ...
    def retrieve(
        ...
        keyword_results = self.keyword_searcher.search(query, limit=rerank_k)
        keyword_dict = {chunk_id: score for chunk_id, score in keyword_results}
        ...
        if semantic_results and semantic_results[0][1] < threshold:
            logger.info(f"Top semantic score {semantic_results[0][1]:.3f} below threshold {threshold}")
            return []
    ...
    def update_parameters(self, alpha: float = None, lambda_param: float = None):
