...
class HierarchicalChunker:
    ...
    def _chunk_single_page(self, text: str, page_num: int, start_position: int) -> List[Chunk]:
        ...
        while current_pos < len(text):
            ...
            if len(chunk_text.strip()) >= self.min_chunk_size or actual_end >= len(text):
                chunks.append(Chunk(
    ...
    def _find_chunk_boundary(self, text: str, start: int, target_end: int) -> Tuple[str, int]:
        """
        ...
        last_portion = chunk_text[-200:] if len(chunk_text) > 200 else chunk_text
        paragraph_matches = list(re.finditer(self.paragraph_break_pattern, last_portion))
