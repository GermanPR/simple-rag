...
from pdfminer.layout import LTTextContainer, LTTextBox, LTTextLine, LTChar, LAParams
...
class PDFExtractor:
    ...
    def extract_text_by_page(self, pdf_content: bytes) -> List[Tuple[int, str]]:
        ...
        try:
            ...
            for page_num, page_layout in enumerate(extract_pages(pdf_file, laparams=self.laparams), 1):
    ...
    def _clean_text(self, text: str) -> str:
        ...
        if len(lines) > 3:
            ...
            if len(lines[0].strip()) < 50 and not lines[0].strip().endswith('.'):
                lines = lines[1:]
            if len(lines) > 0 and len(lines[-1].strip()) < 50 and not lines[-1].strip().endswith('.'):
                lines = lines[:-1]
