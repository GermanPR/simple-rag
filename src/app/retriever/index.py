...
class DatabaseManager:
    ...
    def get_token_stats(self, token: str) -> tuple[int, int]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT df FROM df WHERE token = ?", (token,))
            row = cursor.fetchone()
    ...
    def get_chunk_info(self, chunk_id: int) -> dict[str, Any] | None:
...
class AsyncDatabaseManager:
    """Async version of DatabaseManager using aiosqlite."""
    ...
    async def add_document(self, filename: str, pages: int) -> int:
        """Add a new document and return its ID."""
        async with aiosqlite.connect(self.db_path) as conn:
    ...
    async def get_embedding(self, chunk_id: int) -> np.ndarray | None:
        """Retrieve embedding for a chunk."""
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute(
                "SELECT vec FROM embeddings WHERE chunk_id = ?", (chunk_id,)
    ...
    async def get_all_embeddings(self) -> list[tuple[int, np.ndarray]]:
        """Get all embeddings as (chunk_id, embedding) pairs."""
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute("SELECT chunk_id, vec FROM embeddings")
    ...
    async def get_token_stats(self, token: str) -> tuple[int, int]:
        """Get document frequency and total chunks for IDF calculation."""
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute("SELECT df FROM df WHERE token = ?", (token,))
            row = await cursor.fetchone()
            df = row[0] if row else 0

            cursor = await conn.execute("SELECT COUNT(*) FROM chunks")
            total_chunks_row = await cursor.fetchone()
            total_chunks = total_chunks_row[0] if total_chunks_row else 0

            return df, total_chunks

    async def get_chunk_tokens(self, chunk_id: int) -> dict[str, float]:
        """Get all tokens and their TF scores for a chunk."""
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute(
                "SELECT token, tf FROM tokens WHERE chunk_id = ?", (chunk_id,)
    ...
    async def get_chunk_info(self, chunk_id: int) -> dict[str, Any] | None:
        """Get chunk information including document details."""
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute(
                """
    ...
    async def get_chunks_info(self, chunk_ids: list[int]) -> list[dict[str, Any]]:
        """Get information for multiple chunks."""
        if not chunk_ids:
            return []
